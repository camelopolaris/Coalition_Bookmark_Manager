...
function RenameSessionModal() {
  const { renameTarget, closeRenameSessionModal, renameSession } = useSessions()
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const result = await renameSession(renameTarget.session_id, { name: name.trim() })

    if (!result.success) {
      setError(result.message || 'Unable to rename session.')
      setIsSubmitting(false)
      return
    }
  ...
  return (
    <div className="bookmark-modal__backdrop" onClick={closeRenameSessionModal}>
      <div
      ...
      >
        <div className="bookmark-modal__header">
          <h2 id="rename-session-title" className="bookmark-modal__title">
            Rename session
          </h2>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bookmark-form__field">
            <label htmlFor="rename-session-name" className="bookmark-form__label">
              Session name
            </label>
            <input
